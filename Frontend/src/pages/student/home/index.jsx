    ...
    <div className="min-h-screen bg-white">
      <section className="flex flex-col lg:flex-row items-center justify-between py-8 px-4 lg:px-8">
        ...
        <div className="lg:w-full mb-8 lg:mb-0">
          <img src="https://i.pinimg.com/736x/46/e1/bc/46e1bce13e01cb93b7f8e435019c3e41.jpg" alt="banner" className="h-[80vh] rounded-lg shadow-lg" />
        </div>
      </section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {
            courseCategories?.map((item, index) => {
              return <Button className="justify-start" variant="outline" key={index}>{item?.label}</Button>
            })
          }
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {
            studentViewCourseList && studentViewCourseList?.length > 0 ? studentViewCourseList?.map(courseItem =>
              <div className="border rounded-lg overflow-hidden shadow cursor-pointer" key={courseItem?.title}>
                <img src={courseItem?.image} width="300" height="150" className="w-full h-40 object-cover" />

                <div className="p-4">
                  <h3>{courseItem?.title}</h3>
                  <p className="text-sm text-gray-700 mb-2">{courseItem.instuctorName}</p>
